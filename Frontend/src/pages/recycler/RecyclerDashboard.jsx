...
const RecyclerDashboard = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        // Get overview + monthly trend at the same time
  ...
  const stats = useMemo(() => {
    // Only pending requests count for the first card
    ...
    <main className="flex-1 bg-[#f5f7fb]">
      ...
      <section className="max-w-[1400px] mx-auto py-10 space-y-8">
        ...
        {loadingAnalytics ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
            {/* Total recycled/reused weight */}
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Total Weight</p>
                  <h3 className="text-3xl font-semibold text-slate-800 mt-2">
                    {overview?.totalWeightKg?.toFixed?.(1) ?? "0.0"} kg
                  </h3>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-3xl font-semibold text-slate-800 mt-2">
                    {overview?.totalCo2SavedKg?.toFixed?.(1) ?? "0.0"} kg
                  </h3>
                </div>
            ...
            {/* Current month weight */}
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">This Month</p>
                  <h3 className="text-3xl font-semibold text-slate-800 mt-2">
                    {overview?.thisMonthWeightKg?.toFixed?.(1) ?? "0.0"} kg
                  </h3>
                </div>
                <Truck className="w-10 h-10 text-[#0f55a7]" />
              </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-3xl font-semibold text-slate-800 mt-2">
                    {overview?.progressPercent?.toFixed?.(0) ?? "0"}%
                  </h3>
                </div>
                <CheckCircle2 className="w-10 h-10 text-[#4db848]" />
              </div>
            ...
            <div className="w-full h-[320px] min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={monthlyTrend}>
                  ...
                  {/* Main trend line for recycled weight */}
                  <Line
                    type="monotone"
                    dataKey="weightKg"
                    stroke="#0f55a7"
